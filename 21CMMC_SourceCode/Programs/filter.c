...
#pragma omp for 
  for (n_x=0; n_x<DIM; n_x++){
	...
	if (filter_type == 0){ // real space top-hat
	  if (kR > 1e-4){
	    box[C_INDEX(n_x, n_y, n_z)][0] *= 3.0 * (sin(kR)/pow(kR, 3) - cos(kR)/pow(kR, 2));
	    box[C_INDEX(n_x, n_y, n_z)][1] *= 3.0 * (sin(kR)/pow(kR, 3) - cos(kR)/pow(kR, 2));
	  }
	}
	else if (filter_type == 1){ // k-space top hat
	  kR *= 0.413566994; // equates integrated volume to the real space top-hat (9pi/2)^(-1/3)
	  if (kR > 1){
	    box[C_INDEX(n_x, n_y, n_z)][0] = 0;
	    box[C_INDEX(n_x, n_y, n_z)][1] = 0;
	  }
	}
	else if (filter_type == 2){ // gaussian
	  kR *= 0.643; // equates integrated volume to the real space top-hat
	  box[C_INDEX(n_x, n_y, n_z)][0] *= pow(E, -kR*kR/2.0);
	  box[C_INDEX(n_x, n_y, n_z)][1] *= pow(E, -kR*kR/2.0);
	}
	else{
